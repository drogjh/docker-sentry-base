...
mysql = env('SENTRY_MYSQL_HOST') or 'mysql'
if mysql:
    DATABASES = {
        'default': {
            'ENGINE': 'django.db.backends.mysql',
            'NAME': (
                env('SENTRY_DB_NAME') or env('MYSQL_ENV_MYSQL_USER') or 'sentry'
            ),
            'USER': (
                env('SENTRY_DB_USER') or env('MYSQL_ENV_MYSQL_USER') or 'sentry'
            ),
            'PASSWORD': (
                env('SENTRY_DB_PASSWORD') or env('MYSQL_ENV_MYSQL_PASSWORD') or ''
            ),
            'HOST': mysql,
            'PORT': (
                env('SENTRY_MYSQL_PORT') or '3306'
            ),
            'OPTIONS': {
                'autocommit': True,
            },
